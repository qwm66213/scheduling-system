...
router.get('/', async (req, res) => {
  try {
    ...
    // 频率限制检查
    const rateLimitKey = `personal_${req.storeId}_${start_date}_${end_date}`;
    if (!rateLimit(rateLimitKey)) {
      console.log('[PersonalSummary] Rate limited, returning cached or empty');
      const cachedData = getFromCache(getCacheKey('personal', { store_id: req.storeId, start_date, end_date }));
      return res.json(response(1, '获取成功', cachedData || []));
    }

    // 检查缓存
    const cacheKey = getCacheKey('personal', { store_id: req.storeId, start_date, end_date });
    const cachedData = getFromCache(cacheKey);
    if (cachedData) {
      console.log('[PersonalSummary] Returning cached data');
      return res.json(response(1, '获取成功', cachedData));
    }
